...
function projectObject(includePdf=true){saveInk();return{id:state.id,title:$("projectTitle").value||"無題",category:$("category").value,grade:$("grade").value,domain:$("domain").value,unit:$("unit").value,purpose:$("purpose").value,difficulty:$("difficulty").value,tags:$("tags").value,transcript:$("transcript").value,summary:$("summary").value,sourceProblem:$("sourceProblem").value,variationResults:$("variationResults").value,pdfName:state.pdfName,resourceType:state.resourceType,resourceMime:state.resourceMime,pdfData:includePdf&&state.pdfData?Array.from(new Uint8Array(state.pdfData)):null,annotations:state.annotations,updatedAt:new Date().toISOString()}}

...
const projectFields=["projectTitle","category","grade","domain","unit","purpose","difficulty","tags","transcript","summary","sourceProblem","variationResults"];
...
function projectObject(includePdf=true){saveInk();return{id:state.id,title:$("projectTitle").value||"無題",category:$("category").value,grade:$("grade").value,domain:$("domain").value,unit:$("unit").value,purpose:$("purpose").value,difficulty:$("difficulty").value,tags:$("tags").value,transcript:$("transcript").value,summary:$("summary").value,sourceProblem:$("sourceProblem").value,variationResults:$("variationResults").value,pdfName:state.pdfName,pdfData:includePdf&&state.pdfData?Array.from(new Uint8Array(state.pdfData)):null,annotations:state.annotations,updatedAt:new Date().toISOString()}}
...
async function renderArchive(){const q=$("archiveSearch").value.toLowerCase(),category=$("archiveCategory").value,grade=$("archiveGrade").value,difficulty=$("archiveDifficulty").value,items=(await dbAll()).sort((a,b)=>(b.updatedAt||"").localeCompare(a.updatedAt||""));fillArchiveFilter("archiveCategory",items,"category","すべての教科");fillArchiveFilter("archiveGrade",items,"grade","すべての学年");fillArchiveFilter("archiveDifficulty",items,"difficulty","すべての難易度");const box=$("archiveList");box.innerHTML="";for(const p of items.filter(x=>{const hay=[x.title,x.category,x.grade,x.domain,x.unit,x.purpose,x.difficulty,x.tags,x.transcript,x.summary,x.sourceProblem,x.variationResults].join(" ").toLowerCase();return hay.includes(q)&&(!category||x.category===category)&&(!grade||x.grade===grade)&&(!difficulty||x.difficulty===difficulty)})){const d=document.createElement("article");d.className="archive-item";const classification=[p.category,p.grade,p.domain,p.unit,p.purpose,p.difficulty].filter(Boolean).join(" › ");d.innerHTML=`<h3>${esc(p.title)}</h3><div class="meta">${esc(classification||p.category||"未分類")}<br>${new Date(p.updatedAt).toLocaleString("ja-JP")}<br>${esc(p.tags||"")}</div><p>${esc(p.sourceProblem||p.transcript||p.summary||"本文なし")}</p><div class="actions"><button data-open>開く</button><button data-export>書出し</button><button data-delete>削除</button></div>`;d.querySelector("[data-open]").onclick=()=>openProject(p.id);d.querySelector("[data-export]").onclick=()=>downloadJSON(p);d.querySelector("[data-delete]").onclick=async()=>{if(confirm("削除しますか？")){await dbDelete(p.id);renderArchive()}};box.appendChild(d)}if(!box.children.length)box.innerHTML='<p>該当する教材はありません。</p>'}
["archiveSearch","archiveCategory","archiveGrade","archiveDifficulty"].forEach(id=>$(id).oninput=renderArchive);
async function openProject(id){const p=await dbGet(id);state.id=p.id;state.pdfName=p.pdfName||"";state.pdfData=p.pdfData?new Uint8Array(p.pdfData).buffer:null;state.annotations=p.annotations||{};$("projectTitle").value=p.title||"無題";$("category").value=p.category||"数学";$("grade").value=p.grade||"";$("domain").value=p.domain||"";$("unit").value=p.unit||"";$("purpose").value=p.purpose||"";$("difficulty").value=p.difficulty||"";$("tags").value=p.tags||"";$("transcript").value=p.transcript||"";$("summary").value=p.summary||"";$("sourceProblem").value=p.sourceProblem||"";$("variationResults").value=p.variationResults||"";if(state.pdfData){state.pdf=await pdfjsLib.getDocument({data:state.pdfData.slice(0)}).promise;state.page=1;$("pageCount").textContent=state.pdf.numPages;$("pdfEmpty").style.display="none";await renderPage()}else{state.pdf=null;$("pageNum").textContent="0";$("pageCount").textContent="0";$("pdfEmpty").style.display="block"}document.querySelector('[data-tab="studio"]').click();toast("教材を開きました")}
...
async function askAi({instruction,text,button,statusText="ChatGPTが作成中…"}){
  const endpoint=$("apiEndpoint").value.trim();
  if(!endpoint)throw new Error("設定画面でAIサーバーURLを入力してください");
  button.disabled=true;setStatus(statusText);
  try{
    const r=await fetch(endpoint,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({instruction,text,title:$("projectTitle").value})});
